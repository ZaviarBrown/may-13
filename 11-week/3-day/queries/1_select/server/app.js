...
app.get('/puppies', async (req, res, next) => {
    const allPuppies = await Puppy.findAll({ order: ['name'] });
    // const allPuppies = await Puppy.findAll({
    //     order: [['microchipped'], 'name', 'DESC'], //! No such column Puppy.DESC
    // });
    // const allPuppies = await Puppy.findAll({ order: [['name', 'DESC']] });
...
app.get('/puppies/name/:name', async (req, res, next) => {
    ...
    const puppyByName = await Puppy.findOne({ where: { name } });
    // const puppyByName = await Puppy.findOne({ where: req.params });
    // const puppyByName = await Puppy.findAll({ where: { name } });
