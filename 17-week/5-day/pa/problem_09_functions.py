...
#
#      - A protected property named "pizzas_made" initialized to 0, that is
#        externally accessed only through your getter method.
#
...
#     - arg3 must be a non-negative int, arg3 must not be greater than 10.
#
# Return the error attribute if arguments don't match your constraints.
#
# Additionally, ensure that your returned string follows these rules:
#     - arg1 must be sentence cased (you can expect a string with no spaces)
#     - arg2 must be lowercased
#
# ----- instance method 2: ------
# This method will make a single pizza and give you employee information for
# that order.
#
# Define an instance method named "pizza_production" that takes 3 keyword
...
#
#     "{employee_name} started making pizza at {time_started} and ended at
#      {time_ended}."
#
# This function will increment your pizzas_made property.
#
# ----- instance method 3: ------
# This method will make a large amount of pizzas and return an object compatible
# with a database(JSON).
# For this method, use your knowledge of python's "*args" parameter functionality.
#
# Define an instance method named "pizza_prepper" that collects any number of
# arguments of data type int,
...
#     }
#
# *NOTE: The key of each pizza object represent the order number, and should be
# *an index value.
# *Notice order_number is a str type in the format example above. The reason
# *this is converted is to avoid an error when unpacking later. It's a python thing.
#
...
#     - parameters are non-negative ints and cannot be greater than 10.
#
# Return the error attribute if arguments don't match your constraints.
#
#
...
class PizzaProcessor:
    ...
    def pizza_pick(self, name, crust_type, num_toppings):
        if (
            ...
            or not isinstance(crust_type, str)
            or not isinstance(num_toppings, int)
            or num_toppings > 10
            or num_toppings < 0
        ):
            return self._error
        ...
        return (
            f"{name} ordered a {crust_type} crust pizza with {num_toppings} toppings!"
        )

    def pizza_production(self, time_started=None, time_ended=None, employee_name=None):
        if (
            not isinstance(time_started, str)
            or not isinstance(time_ended, str)
    ...
    def pizza_prepper(self, *toppings):
        ...
        for i, top_num in enumerate(toppings):
            pizza_dict[str(i)] = {
                "crust": "stuffed" if i % 2 else "thin",
                "toppings": top_num,
            }
            self._pizzas_made += 1
    ...
    def pizza_printer(self, **pizza_orders):
        starter_str = "Printing all the pizzas!"

        for number, pizza in pizza_orders.items():
            crust, toppings = pizza.values()
            starter_str += (
                f"\nOrder {number} is a {crust} crust with {toppings} toppings\n"
            )
...
# # ? Good
# print(
#     pizza_processor.pizza_production(
#         employee_name="Brad", time_started="03:00pm", time_ended="03:30pm"
#     )
# )
