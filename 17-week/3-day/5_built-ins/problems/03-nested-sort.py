...
teachers = [
    {
        ...
        "years_active": 5,
        "classroom": {"building_id": "A", "room_number": 12, "capacity": 45},
    },
    {
        "name": "Richard Emilyson",
        "subjects": ["English 11", "AP English Language"],
        "years_active": 12,
        "classroom": {"building_id": "J", "room_number": 42, "capacity": 60},
    },
    {
        "name": "Richly Emiardson",
        "subjects": ["Chemistry", "Chemistry Honors", "AP Chemistry"],
        "years_active": 8,
        "classroom": {"building_id": "C", "room_number": 5, "capacity": 32},
    },
]
...
def sort_teachers_by_classroom_capacity(teacher_list):
    return list(
        map(
            lambda el: el["name"],
            sorted(teacher_list, key=lambda el: el["classroom"]["capacity"]),
        )
    )
...
#     return [teacher["name"] for teacher in sorted(card_list, key=capacity)]
